...
const TEAM_ALIASES = {
  ...
  nzl:["new zealand"],hat:["haiti"],cur:["curacao"],sen:["senegal"],
};

const NAME_TO_ID = {};
for (const [id, aliases] of Object.entries(TEAM_ALIASES)) {
  for (const alias of aliases) NAME_TO_ID[norm(alias)] = id;
}
...
function findScorerId(playerName) {
  ...
  for (const [id, aliases] of Object.entries(SCORER_ALIASES)) {
...
// Rank = how far a team progressed. Loser at each round gets that round's rank.
// Winner gets rank+1 (or 6 for Final winner).
  ...
  try {
    ...
    for (const group of sd?.standings || []) {
      ...
      for (const row of group.table || []) {
        const id = findTeamId(row.team?.name);
        if (!id) continue;
    ...
    // Knockout stages
    const kt = {}; // id -> { stageRank, qualified }
    const ensure = id => { if (!kt[id]) kt[id] = { stageRank: 0, qualified: false }; };

    for (const match of md?.matches || []) {
      const rank = STAGE_RANK[match.stage];
      if (!rank) continue;
      const hId = findTeamId(match.homeTeam?.name);
      const aId = findTeamId(match.awayTeam?.name);
      ...
      if (wId) {
        ...
        const wr = match.stage === "FINAL" ? 6 : rank + 1;
        if (wr > kt[wId].stageRank) kt[wId].stageRank = wr;
      }
    }
    ...
    // Scorers — goals only, never touch .boot
    for (const { player, goals } of scd?.scorers || []) {
      const id = findScorerId(player?.name);
      if (!id) continue;
