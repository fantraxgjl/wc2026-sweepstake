...
// Pull fixtures/standings/scorers from football-data.org and fold them into the stored
// state. Returns {ok, ...} describing the outcome; shared by the scheduled job and the
// admin "Sync now" endpoint.
export async function runSync(){
  if(!API_KEY){console.log("[sync] No API key");return{ok:false,reason:"no-api-key"};}
  const store=getStore({name:"wc26",consistency:"strong"});
  const state=await store.get("state",{type:"json"});
  if(!state?.setupComplete){console.log("[sync] Not set up");return{ok:false,reason:"not-set-up"};}
  ...
  try{
    ...
    if(!sRes.ok||!mRes.ok||!scRes.ok){
      console.error("[sync] API error",sRes.status,mRes.status,scRes.status);
      return{ok:false,reason:"api-error",status:[sRes.status,mRes.status,scRes.status]};
    }
    ...
    for(const match of md?.matches||[]){
      ...
      // (winning the FINAL = champion). The only points a loser banks is runner-up for
      // losing the FINAL — earlier-round losers keep just the rounds they already won.
    ...
    const newState={...state,teamScores,scorerScores,lastSynced};
    await store.setJSON("state",newState);
    console.log("[sync] Done",lastSynced);
    return{ok:true,lastSynced,state:newState};
  }catch(err){
